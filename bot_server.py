...
# Цены в дампе aleado (start_price_num/end_price_num) хранятся в ТЫСЯЧАХ
# иен (千円), а не в иенах напрямую. Подтверждено диагностикой на живых
# данных: (1) сам отчёт по лоту (BDS report) для истории продаж прямым
# текстом пишет "... the lowest price 0 thousand jpy ..." — то есть
# источник сам описывает эти цифры как "тысячи иен"; (2) если умножить
# сырое значение на 1000, у ВСЕХ аукционных систем в фиде (BEENET, JBA,
# ARAI) типичная цена попадает в правдоподобный для мотоцикла диапазон
# (десятки тысяч — несколько миллионов иен), а без умножения получаются
# абсурдные "цена 50 иен", "цена 100 иен" и т.п. Раньше бот показывал
# сырое значение как есть, из-за чего цена была занижена ровно в 1000
# раз (жалоба живого пользователя: бот показал "Цена: ¥600 (≈313 ₽)"
# для лота, у которого на сайте аукциона реальная стартовая цена —
# 600 000 ¥ / 651 339 ₽).
_PRICE_SCALE = 1000


def _is_price_placeholder(jpy_raw):
    """В сыром (ещё не умноженном на _PRICE_SCALE) значении цены часть
    строк вместо реальной цены содержит служебное число-заглушку "цена
    не указана" — например, у аукционной системы JBA почти половина
    строк содержит буквально 77777, у других встречаются 9999/99999/
    999999/9999999. Отличить такую заглушку от настоящей цены можно по
    виду: реальные цены в этом фиде — 1-4-значные числа (после ×1000 —
    от нескольких тысяч до ~9 999 000 иен), а заглушки — числа из ОДНОЙ
    повторяющейся цифры длиной от 4 знаков и больше. Такое совпадение у
    настоящей цены практически невозможно, а у заглушки — типично."""
    try:
        n = int(jpy_raw)
...
def _lot_price_jpy(lot):
    """Цена лота в ИЕНАХ (уже с поправкой на _PRICE_SCALE — см. выше) —
    или None, если цены нет или сырое значение похоже на заглушку."""
    raw = lot.get("end_price") or lot.get("start_price")
    jpy_raw = parse_price(raw)
    if not jpy_raw or jpy_raw <= 0:
        return None
    if _is_price_placeholder(jpy_raw):
        return None
    return jpy_raw * _PRICE_SCALE
...
def enqueue_lots(bot, chat_id, items):
    ...
    вторую порцию поверх ещё не просмотренной. Используется для ФОНОВЫХ
    уведомлений (check_all) — там естественно ждать своей очереди после
    того, что уже показывается."""
    if not items:
        return
...
def show_preview_now(bot, chat_id, items):
    """Как enqueue_lots, но для МГНОВЕННОГО просмотра по явному запросу
    пользователя (кнопка "Показать, что есть сейчас") — новые лоты
    встают В НАЧАЛО очереди, и показ запускается сразу, даже если в
    очереди уже лежит недосмотренный "хвост" от предыдущего показа.

    Без этого результат явного запроса молча тонул за старой незакрытой
    пагинацией: бот присылал "Сейчас подходит лотов: N", а сам лот
    никогда не показывался — потому что enqueue_lots дописывал его в
    КОНЕЦ очереди и не отправлял ничего, пока пользователь не долистает
    более старую порцию до конца кнопками "Показать ещё"/"Прекратить
    показ" (жалоба живого пользователя: сохранил фильтр, нажал "Показать,
    что есть сейчас" — бот написал "Сейчас подходит лотов: 1" и на этом
    всё, самого лота не было)."""
    if not items:
        return
    existing = PENDING_LOTS.get(chat_id) or []
    PENDING_LOTS[chat_id] = items + existing
    send_lots_batch(bot, chat_id)
...
def send_preview(context, chat_id, watch):
    ...
    show_preview_now(context.bot, chat_id, items)
